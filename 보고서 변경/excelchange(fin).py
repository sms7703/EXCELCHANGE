...
# 정해진 패스워드 (대소문자 구분 없이)
correct_password = "ICT"

# 패스워드 입력받기
while True:
    password = getpass.getpass("패스워드를 입력하세요: ")
    if password.upper() == correct_password.upper():
        break
    else:
...
# 최종 확인 시에만 작업 전 작업 후 경로를 나타내도록 변수 설정
show_paths = False
...
while True:
    ...
    # 최종 확인 시에만 작업 전 작업 후 경로를 나타내도록 변수 업데이트
    show_paths = True

    # 변경 내용을 마지막으로 확인
    if show_paths:
        print("작업 전 파일 경로:", source_directory)
        print("작업 후 파일 경로:", destination_directory)
    ...
    if confirm == 'y':
        ...
        with open(log_file_path, 'a') as log_file:  # 'a' 모드로 열어서 뒤에 추가
            ...
            if show_paths:  # 최종 확인 시에만 작업 전 작업 후 경로 추가
                log_file.write(f"작업 전 파일 경로: {source_directory}\n")
                log_file.write(f"작업 후 파일 경로: {destination_directory}\n")
            for cell_coord, new_value in replacement.items():
...
"""
    pip 필요 리스트
        ...
        #pyinstaller --onefile excelchange(fin).py  
        #pyinstaller --key="12345" excelchange(fin).py (불가)
        pyinstaller --onefile --icon=gs.ico excelchange(fin).py 아이콘 넣는거
