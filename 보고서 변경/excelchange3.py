...
while True:
    ...
    for cell_coord, new_value in replacement.items():
        ...
        # 작업 전 파일 경로 추가
        original_file_path = os.path.join(source_directory + "\\")
        print(f"작업 전 파일 경로: {original_file_path}")

        # 작업 후 파일 경로 추가
        new_file_path = os.path.join(destination_directory + "\\")
        print(f"작업 후 파일 경로: {new_file_path}")
    ...
    if confirm == 'y':
        ...
        with open(log_file_path, 'a') as log_file:  # 'a' 모드로 열어서 뒤에 추가
            ...
            for cell_coord, new_value in replacement.items():
                ...
                # 작업 전 파일 경로 추가
                original_file_path = os.path.join(source_directory + "\\")
                log_file.write(f"작업 전 파일 경로: {original_file_path}\n")

                # 작업 후 파일 경로 추가
                new_file_path = os.path.join(destination_directory+ "\\")
                log_file.write(f"작업 후 파일 경로: {new_file_path}\n")
